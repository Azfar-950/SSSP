...
Graph read_subgraph(const string &filename, int rank, ofstream &log_file)
{
    Graph graph;
...
void read_nodes(const string &filename, int rank, set<int> &local_vertices, set<int> &ghost_vertices, int &max_vertex, ofstream &log_file)
{
    ifstream file(filename);
    if (!file.is_open())
    {
...
vector<ChangeOperation> read_changes(const string &filename, int rank, ofstream &log_file)
{
    ...
    if (rank == 0)
    {
        log_file << "[Rank " << rank << "] Read " << changes.size() << " changes." << endl;
    }
    return changes;
}
                           ...
                           ofstream &log_file)
{
    ...
    for (int i = 0; i < max_messages; ++i)
    {
        ...
        log_file << "[Rank " << rank << "] Received GHOST_UPDATE for vertex " << global_id << " from rank " << status.MPI_SOURCE << endl;
        ...
        if (ghost_distances.count(global_id) && global_to_owner_map.at(global_id) != rank)
        {
            if (new_dist < ghost_distances[global_id])
            {
                ...
                ghost_change = true;
                log_file << "[Rank " << rank << "] Updated ghost distance for vertex " << global_id << " to " << new_dist << endl;
                      ...
                      ofstream &log_file)
{
    ...
    read_nodes(nodes_file, rank, local_vertices, ghost_vertices, max_vertex, log_file);
    ...
    // Read local subgraph
    local_graph = read_subgraph(subgraph_file, rank, log_file);
    ...
    for (const auto &change : changes)
    {
        if (local_vertices.count(change.u) || local_vertices.count(change.v))
        {
            if (change.is_deletion)
            {
                ...
                log_file << "[Rank " << rank << "] Removed edge " << change.u << "->" << change.v << endl;
            }
            else
            {
                local_graph.add_edge(change.u, change.v, change.weight);
                log_file << "[Rank " << rank << "] Added edge " << change.u << "->" << change.v << " (weight " << change.weight << ")" << endl;
            }
        }
    }
    ...
    for (int r = 0; r < size; ++r)
    {
        ...
        int rank_max_vertex;
        read_nodes("subgraph_" + to_string(r) + "_nodes.txt", rank, rank_local_vertices, rank_ghost_vertices, rank_max_vertex, log_file);
    ...
    // Debug local graph
    log_file << "[Rank " << rank << "] Local graph edges after changes:" << endl;
    for (int u : local_vertices)
    {
        for (const auto &edge : local_graph.get_edges(u))
        {
            log_file << "[Rank " << rank << "] Edge " << u << "->" << edge.to << " (weight " << edge.weight << ")" << endl;
        }
    }
    log_file << "[Rank " << rank << "] Graph distribution completed: " << local_vertices.size() << " local vertices, "
                     ...
                     ofstream &log_file)
{
    ...
    log_file << "[Rank " << rank << "] SSSP initialization complete. Local vertices: ";
    for (int u : local_vertices)
        log_file << u << " ";
    log_file << "\n[Rank " << rank << "] Initial ghost distances: ";
    for (int u : ghost_vertices)
        log_file << u << ":" << (ghost_distances[u] == INF ? "INF" : to_string(ghost_distances[u])) << " ";
    log_file << endl;
}
                         ...
                         ofstream &log_file)
{
    ...
    log_file << "[Rank " << rank << "] Local graph edges after applying changes:" << endl;
    ...
    log_file << "[Rank " << rank << "] Applied relevant changes." << endl;
}
                             ...
                             ofstream &log_file)
{
    ...
    process_ghost_updates(rank, size, global_to_local_map, global_to_owner_map,
                          ...
                          local_vertices, ghost_vertices, local_graph, log_file);

    local_change = false;
    vector<int> updated_vertices;
    map<int, bool> new_affected;
    set<int> sent_updates;
     vector<int> local_vertices_vec(local_vertices.begin(), local_vertices.end());

    #pragma omp parallel for schedule(dynamic)
    for (auto it = 0; it < local_vertices_vec.size(); ++it)
    {
        int v_global = local_vertices_vec[it];
                ...
                if (edge.to == v_global)
                {
                    ...
                    if (dist_u != INF && dist_v > dist_u + weight)
                    {
                        #pragma omp critical
                        {
                            ...
                            vertex_changed = true;
                            local_change = true;
                            log_file << "[Rank " << rank << "] Relax " << u_global << "->" << v_global
            ...
            if (dist_v != INF && dist_w > dist_v + weight)
            {
                #pragma omp critical
                {
                    ...
                    if (global_to_local_map.count(w_global))
                    {
                        ...
                        vertex_changed = true;
                        local_change = true;
                        log_file << "[Rank " << rank << "] Relax " << v_global << "->" << w_global
                    ...
                    else
                    {
                        if (ghost_distances[w_global] > new_dist)
                        {
                            ...
                            vertex_changed = true;
                            local_change = true;
                            log_file << "[Rank " << rank << "] Updated ghost distance for vertex " << w_global
                                     << " to " << new_dist << endl;
                        }
                    }
                }
        ...
        if (sent_updates.find(u_global) == sent_updates.end())
        {
            ...
            for (int r = 0; r < size; ++r)
            {
                if (r != rank)
                {
                    ...
                    log_file << "[Rank " << rank << "] Sending GHOST_UPDATE for vertex " << u_global << " to rank " << r << endl;
                }
            }
        }
    }
    ...
    log_file << "[Rank " << rank << "] Distances: ";
    for (const auto &[v, d] : distance)
    {
        log_file << v << ":" << (d == INF ? "INF" : to_string(d)) << " ";
    }
    log_file << endl;
    log_file << "[Rank " << rank << "] Ghost distances: ";
    for (const auto &[v, d] : ghost_distances)
    {
        log_file << v << ":" << (d == INF ? "INF" : to_string(d)) << " ";
    }
    log_file << endl;
    log_file << "[Rank " << rank << "] Affected vertices: ";
    ...
    log_file << endl;
    log_file << "[Rank " << rank << "] local_change: " << (local_change ? "YES" : "NO") << ", ghost_change: " << (ghost_change ? "YES" : "NO") << endl;
}
                        ...
                        ofstream &log_file)
{
    ...
    if (rank == 0)
    {
        ...
        out_file.close();
        log_file << "[Rank 0] Modified graph saved to " << filename << endl;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    // Initialize log file for each rank
    ofstream log_file("rank_" + to_string(rank) + "_log.txt");
    if (!log_file.is_open())
    {
        cerr << "[Rank " << rank << "] Error: Could not open log file rank_" << rank << "_log.txt" << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    vector<ChangeOperation> changes = read_changes(changes_file, rank, log_file);
    ...
    distribute_graph(rank, size, subgraph_file, nodes_file, changes, global_to_local_map, global_to_owner_map,
                     local_vertices, ghost_vertices, local_graph, max_vertex, num_global_vertices, log_file);
    ...
    map<int, bool> affected;
    initialize_sssp(rank, source, local_vertices, ghost_vertices, distance, parent, ghost_distances, affected, log_file);

    apply_changes_step1(rank, size, changes, global_to_owner_map, local_vertices, local_graph, affected, log_file);
    ...
    while (global_change && iteration < MAX_ITERATIONS)
    {
        ...
        relax_edges_distributed(rank, size, local_graph, global_to_local_map,
                                ...
                                local_vertices, ghost_vertices, log_file);
        ...
        iteration++;
        log_file << "[Rank " << rank << "] Iteration " << iteration << " completed. Global change: "
    ...
    if (rank == 0)
    {
        ...
        out_file.close();
        log_file << "[Rank 0] Performance data appended to performance_log.txt" << endl;
    }

    log_file << "[Rank " << rank << "] Final ghost distances: ";
    for (const auto &[v, d] : ghost_distances)
    {
        log_file << v << ":" << (d == INF ? "INF" : to_string(d)) << " ";
    }
    ...
    if (rank == 0)
    {
        log_file << "*** Final SSSP Distances ***" << endl;
        for (int u = 0; u < num_global_vertices; ++u)
        {
            if (global_distances[u] != INF)
            {
                log_file << "Vertex " << u << ": Distance = " << global_distances[u]
