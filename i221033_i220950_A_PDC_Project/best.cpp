                             ...
                             const set<int> &ghost_vertices)
{
    ...
    // Copy local_vertices to a vector for OpenMP parallelization
    vector<int> local_vertices_vec(local_vertices.begin(), local_vertices.end());

    #pragma omp parallel for schedule(dynamic)
    for (size_t i = 0; i < local_vertices_vec.size(); ++i)
    {
        int v_global = local_vertices_vec[i];
...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        cout << "[Rank 0] Set OpenMP threads to " << omp_get_max_threads() << " per process" << endl;
    }
    ...
    string subgraph_file = "subgraph_" + to_string(rank) + ".txt";// subgraph_0.txt
    ...
    // In main(), after distribute_graph call
    ...
    double execution_time = chrono::duration<double>(end_time - start).count();

    // Log performance data on rank 0
    

    //std::cout << "Time taken: " << execution_time << " seconds" << std::endl;
    if (rank == 0)
    {
    ...
    std::cout << "*** Final SSSP Distances ***" << std::endl;
    if (rank == 0)
    {
        //std::ofstream out_file("sssp_output.txt");
        cout << "vertex distance parent\n";
        for (int u = 0; u < num_global_vertices; ++u)
        {
            if (global_distances[u] != INF)
            {
                cout << u << " " << global_distances[u] << " " << global_parents[u] << "\n";
            }
        }
        //out_file.close();
        std::cout << "******" << std::endl;
