...
public:
    ...
    void remove_edge(int from, int to) {
        ...
        reverse_edges.erase(remove_if(reverse_edges.begin(), reverse_edges.end(),
                                      [from](const Edge& e) { return e.to == from; }), reverse_edges.end());
    }
...
Graph read_subgraph(const string& filename, int rank) {
    ...
    if (!file.is_open()) {
        if (rank == 0) cerr << "Error opening subgraph file: " << filename << endl;
...
void read_nodes(const string& filename, int rank, set<int>& local_vertices, set<int>& ghost_vertices, int& max_vertex) {
    ifstream file(filename);
    if (!file.is_open()) {
        if (rank == 0) cerr << "Error opening nodes file: " << filename << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    string line;
...
vector<ChangeOperation> read_changes(const string& filename, int rank) {
    ...
    ifstream file(filename);
    if (!file.is_open()) {
        if (rank == 0) cerr << "Error opening changes file: " << filename << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    string line;
    while (getline(file, line)) {
...
// Distribute graph using METIS files
...
void initialize_sssp(int rank, int source, const set<int>& local_vertices,
    ...
    for (int u : local_vertices) cout << u << " ";
    cout << "\n[Rank " << rank << "] Initial ghost distances: ";
    for (int u : ghost_vertices) cout << u << ":" << (ghost_distances[u] == INF ? "INF" : to_string(ghost_distances[u])) << " ";
    cout << endl;
}
